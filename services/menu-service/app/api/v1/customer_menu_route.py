...
@router.get("/internal/menu-items/{menu_item_id}", response_model=InternalMenuItemResponse)
async def get_internal_menu_item(menu_item_id: UUID, db: AsyncSession = Depends(get_db)):
    result = await service.get_menu_item_internal(db, menu_item_id)
    return result